  ...
  return function (tree) {
    visit(tree, 'element', function (node, index, parent) {
      if (
        ...
        // for all URLs
        // link properties
        const properties = createIfNeeded(settings.properties, settings, node)
        if (properties) {
          Object.assign(node.properties, structuredClone(properties))
        }
        // link content and content properties
        const contentRaw = createIfNeeded(settings.content, settings, node)
        const content =
          contentRaw && !Array.isArray(contentRaw) ? [contentRaw] : contentRaw
        if (content) {
          const contentProperties =
            createIfNeeded(settings.contentProperties, settings, node) || {}
        ...
        if (isAbsolute) {
          ...
          // link rel
          const relRaw = createIfNeeded(settings.rel, settings, node) || defaultRel
          ...
          // link target
          const target = createIfNeeded(settings.target, settings, node)
          if (target) {
 ...
 *   Value.
 * @param settings
 *   Settings object that was passed in.
 * @param {Element} element
 *   Element.
 * @returns {T extends Function ? ReturnType<T> : T}
 *   Result.
 */
function createIfNeeded(value, settings, element) {
  return typeof value === 'function' ? value(element, settings) : value
}
